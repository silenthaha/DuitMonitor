...
const supabaseKey = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imd1bGZvd3hzaGhhcWxpYm1rYmtzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjA0MDEzMTQsImV4cCI6MjA3NTk3NzMxNH0.hQ8FVqBjgYq4EzT3bx2vZHp9bg19E-ml-qR8nUP9JSw";
...
function renderDashboard(dateFilter = null) {
    // --- Kiraan Jumlah Semua Baki Aktif ---
    const totalBalance = allWallets
        .filter((wallet) => !wallet.is_hidden)
        .reduce((sum, wallet) => sum + wallet.balance, 0);
    document.getElementById("total-income").textContent = `RM ${totalBalance.toFixed(2)}`;

    // --- Kiraan Jumlah Perbelanjaan (boleh ditapis) ---
...
function setDefaultExpenseFilter() {
    ...
    if (fromDateInput && toDateInput) {
        ...
        renderDashboard({ dateFrom: fromDateInput.value, dateTo: toDateInput.value });
    }
}
...
function renderTransactionTable() {
    ...
    container.innerHTML = createResponsiveTable(data, cols, mobileCols, "transactions");
}

function renderSubscriptionTable() {
    const container = document.getElementById("subscription-list");
    const data = allTransactions.filter((t) => t.category === "subscription").slice(0, 5);
    const cols = ["Item", "Tarikh", "End Date", "Kos", "Status"];
    const mobileCols = ["Item", "End Date", "Kos"];
    container.innerHTML = createResponsiveTable(data, cols, mobileCols, "subscriptions");
}

function renderCommitmentTable() {
    const container = document.getElementById("commitment-list");
    const data = allTransactions.filter((t) => t.category === "monthly commitment").slice(0, 5);
    const cols = ["Item", "Kos", "Status", "Tarikh Bayaran"];
    const mobileCols = ["Item", "Kos", "Status"];
    container.innerHTML = createResponsiveTable(data, cols, mobileCols, "commitments");
}

function renderCreditCardTable() {
    const container = document.getElementById("credit-card-list");
    const creditWallets = allWallets.filter((w) => w.type === "credit_card").map((w) => w.name);
    const data = allTransactions.filter((t) => creditWallets.includes(t.source)).slice(0, 5);
    const cols = ["Item", "Jenis", "Nama Kad", "Kos", "Tarikh", "Baki"];
    const mobileCols = ["Item", "Nama Kad", "Kos"];
    container.innerHTML = createResponsiveTable(data, cols, mobileCols, "credit-cards");
}
...
function renderDebtList() {
    ...
    if (!data || data.length === 0) {
        container.innerHTML = '<p class="p-4 text-gray-500">Tiada rekod hutang Pay Later.</p>';
        return;
    }
    const cols = ["Item", "Kos", "Jenis Pembayaran", "Tarikh", "Jatuh Tempo", "Status"];
    const mobileCols = ["Item", "Jatuh Tempo", "Status"];
    container.innerHTML = createResponsiveTable(data, cols, mobileCols, "debt-list");
}
...
function renderWalletList() {
    ...
    allWallets.forEach((w) => {
        ...
        html += `<tr class="${rowClass}"><td class="px-5 py-5 border-b border-gray-200 text-sm">${w.name}</td><td class="px-5 py-5 border-b border-gray-200 text-sm">${w.type.replace("_", " ")}</td><td class="px-5 py-5 border-b border-gray-200 text-sm">RM ${w.balance.toFixed(2)}</td><td class="px-5 py-5 border-b border-gray-200 text-sm"><label class="inline-flex items-center"><input type="checkbox" ${isActive ? "checked" : ""} onchange="toggleWalletVisibility('${w.id}', !this.checked)" class="form-checkbox h-4 w-4 text-indigo-600"><span class="ml-2 text-gray-700">Aktif</span></label></td><td class="px-5 py-5 border-b border-gray-200 text-sm"><button onclick="deleteWallet('${w.id}')" class="text-red-600 hover:text-red-900">Delete</button></td></tr>`;
    });
    html += `</tbody></table>`;
...
function createResponsiveTable(data, cols, mobileCols, type) {
    if (!data || data.length === 0) return '<p class="p-4 text-gray-500">Tiada data.</p>';
    ...
    cols.forEach((col) => (html += `<th class="px-5 py-3 border-b-2 border-gray-200 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider hidden md:table-cell">${col}</th>`));
    mobileCols.forEach((col) => (html += `<th class="px-5 py-3 border-b-2 border-gray-200 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider md:hidden">${col}</th>`));
    html += `</tr></thead><tbody>`;
    data.forEach((item, index) => {
        ...
        cols.forEach((col) => { let value = getNestedValue(item, col); html += `<td class="px-5 py-5 border-b border-gray-200 text-sm hidden md:table-cell">${value}</td>`; });
        mobileCols.forEach((col) => { let value = getNestedValue(item, col); html += `<td class="px-5 py-5 border-b border-gray-200 text-sm md:hidden">${value}</td>`; });
        html += `</tr><tr id="detail-${type}-${index}" class="hidden"><td colspan="${cols.length + 1}" class="px-5 py-3 border-b border-gray-200 bg-gray-50 text-sm"><div class="space-y-2">`;
        cols.forEach((col) => { if (!mobileCols.includes(col)) { let value = getNestedValue(item, col); html += `<p><strong>${col}:</strong> ${value}</p>`; } });
...
function getNestedValue(obj, key) {
    ...
    if (lowerKey === "tarikh" || lowerKey === "tarikh bayaran") return new Date(obj.transaction_date).toLocaleDateString("ms-MY");
    if (lowerKey === "end date") return obj.end_date ? new Date(obj.end_date).toLocaleDateString("ms-MY") : "-";
    ...
    if (lowerKey === "tarikh jatuh tempo") return obj.due_date ? new Date(obj.due_date).toLocaleDateString("ms-MY") : "-";
    if (lowerKey === "status") {
    ...
    if (lowerKey === "baki") {
        ...
        const totalExpenses = allTransactions.filter((t) => t.source === obj.source && t.type === "expense").reduce((sum, t) => sum + t.amount, 0);
...
async function handleAddTransaction(e) {
    e.preventDefault();
    const transactionData = { user_id: currentUser.id, item: document.getElementById("item").value, amount: parseFloat(document.getElementById("amount").value), type: document.querySelector('input[name="type"]:checked').value, category: document.getElementById("category").value, source: document.getElementById("source").value, transaction_date: document.getElementById("transaction-date").value, end_date: document.getElementById("end-date").value || null, due_date: document.getElementById("due-date").value || null };
    const { error } = await supabase.from("transactions").insert([transactionData]);
    if (error) alert("Gagal menambah transaksi: " + error.message);
    else { alert("Transaksi berjaya ditambah!"); window.location.href = "index.html"; }
}

async function handleAddWallet(e) {
    e.preventDefault();
    const walletData = { user_id: currentUser.id, name: document.getElementById("wallet-name").value, type: document.getElementById("wallet-type").value, balance: parseFloat(document.getElementById("wallet-balance").value), credit_limit: document.getElementById("wallet-limit").value ? parseFloat(document.getElementById("wallet-limit").value) : null, is_hidden: false };
    const { error } = await supabase.from("wallets").insert([walletData]);
    if (error) alert("Gagal menambah wallet: " + error.message);
    else { e.target.reset(); loadAdminPage(); }
}
...
async function toggleWalletVisibility(id, isHidden) {
    const { error } = await supabase.from("wallets").update({ is_hidden: isHidden }).eq("id", id);
    if (error) { console.error("Error updating visibility:", error); alert("Gagal mengemas kini status."); }
    else { loadAdminPage(); loadMainPage(); }
}

function editTransaction(id) {
    const transaction = allTransactions.find((t) => t.id === id);
    if (!transaction) { alert("Transaksi tidak dijumpai!"); return; }
    ...
    sourceSelect.innerHTML = '<option value="">Pilih Sumber</option>';
    allWallets.forEach((w) => { const option = document.createElement("option"); option.value = w.name; option.textContent = `${w.name} (RM${w.balance})`; sourceSelect.appendChild(option); });
    ...
    if (transaction.category === "subscription") { endDateGroup.classList.remove("hidden"); endDateInput.value = transaction.end_date || ""; } else { endDateGroup.classList.add("hidden"); }
    const dueDateGroup = document.getElementById("edit-due-date-group");
    const dueDateInput = document.getElementById("edit-due-date");
    if (transaction.category === "Pay Later") { dueDateGroup.classList.remove("hidden"); dueDateInput.value = transaction.due_date || ""; } else { dueDateGroup.classList.add("hidden"); }
...
async function handleUpdateTransaction(e) {
    ...
    const updatedData = { item: document.getElementById('edit-item').value, amount: parseFloat(document.getElementById('edit-amount').value), type: document.querySelector('input[name="edit-type"]:checked').value, category: document.getElementById('edit-category').value, source: document.getElementById('edit-source').value, transaction_date: document.getElementById('edit-transaction-date').value, end_date: document.getElementById('edit-end-date').value || null, due_date: document.getElementById('edit-due-date').value || null };
    const { error } = await supabase.from('transactions').update(updatedData).eq('id', id);
    if (error) { alert('Gagal mengemaskini transaksi: ' + error.message); }
    else { alert('Transaksi berjaya dikemaskini!'); closeEditModal(); loadMainPage(); }
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("show-signup")?.addEventListener("click", (e) => { e.preventDefault(); document.getElementById("login-modal").classList.add("hidden"); document.getElementById("signup-modal").classList.remove("hidden"); });
    document.getElementById("show-login")?.addEventListener("click", (e) => { e.preventDefault(); document.getElementById("signup-modal").classList.add("hidden"); document.getElementById("login-modal").classList.remove("hidden"); });
    document.getElementById("logout-btn")?.addEventListener("click", handleLogout);
    document.getElementById("search-toggle")?.addEventListener("click", toggleSearch);
    document.getElementById("apply-search")?.addEventListener("click", applyDateFilter);
    document.getElementById("wallet-form")?.addEventListener("submit", handleAddWallet);
    ...
    document.getElementById("category")?.addEventListener("change", (e) => {
        ...
        if (e.target.value === "subscription") { endDateGroup.classList.remove("hidden"); }
        else if (e.target.value === "Pay Later") { dueDateGroup.classList.remove("hidden"); }
    });
    document.getElementById('expense-filter-form')?.addEventListener('submit', (e) => {
        e.preventDefault();
        const dateFrom = document.getElementById('expense-date-from').value;
        const dateTo = document.getElementById('expense-date-to').value;
        if (!dateFrom || !dateTo) { alert("Sila pilih kedua-dua tarikh."); return; }
